...
float get_point(uint8_t *p, uint8_t rows, uint8_t cols, int8_t x, int8_t y);
void set_point(uint8_t *p, uint8_t rows, uint8_t cols, int8_t x, int8_t y, float f);
void get_adjacents_2d(uint8_t *src, uint8_t *dest, uint8_t rows, uint8_t cols, int8_t x, int8_t y);
...
void loop() {
  ...
  uint8_t temp[AMG_COLS * AMG_ROWS];

  for (int i = 0; i < AMG_COLS * AMG_ROWS; i++)
  {
    temp[i] = ((uint8_t)(temp[i] * 2.0) + 41);
  }
  uint8_t dest_2d[INTERPOLATED_ROWS * INTERPOLATED_COLS];

  interpolate_image(temp, AMG_ROWS, AMG_COLS, dest_2d, INTERPOLATED_ROWS, INTERPOLATED_COLS);
...
void drawpixels(uint8_t *p, uint8_t rows, uint8_t cols) {
  ...
  for (int y = 0; y < rows; y++) {
    for (int x = 0; x < cols; x++) {
      ...
      display.drawPixel(x + XOFF, y + YOFF, colors[val]);
    }
  }
}
